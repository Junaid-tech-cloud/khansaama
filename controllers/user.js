...
module.exports = {
    ...
    signIn: async (req, res, next) => {
        ...
        res.status(200).send({token, user: req.user, lat: req.user.address.coordinates[0], lng: req.user.address.coordinates[1]});
    },

    signUp: async (req, res, next) => {
        console.log(`Sign up API hit`)
        var newUser;
        const {name, email, password, lat, lng, phone, featured, specialities, minOrder, deliveryTime} = req.validated.body;
        if(req.validated.body.role === 'chef' ) {
            ...
            .then((user) => {
                ...
                res.status(400).send({message: "Error processing signup request", err});
            });
        ...
        const user = await Chef.findById(id)
            ...
            if(!user) {
                ...
                .then((customer) => {
                    ...
                    res.status(400).send({message: "error while fetching cust", error: err});
                });
            }
    ...
    findByEmail: async (req, res, next) => {
                ...
                let user = await Chef.findOne({email})
                    .then( (chef) => {
                        console.log(!chef);
                        if(!chef) {
                            return null;
